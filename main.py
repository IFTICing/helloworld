...
from read_imdb import read_imdb
...
def get_dataloader(dir='data/aclImdb'):
    ...
    def collate_fn(batch):
        x, y = zip(*batch) #x（batch个x的张量）和y（batch个对应的标签）都是元组
        x_pad = pad_sequence(x, batch_first=True)
        #按x中的最大的句子长度填充其余小句子，batch_first决定了batch是否在第一位，为Fales则（x_pad）第一位/维度为最大句子长度，第二位/维度为batch
        y = torch.Tensor(y)
        return x_pad, y
    ...
    #由于前一半Dataset标签为1后一半为0，故必须用shuffle
